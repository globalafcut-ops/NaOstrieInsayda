    ...
    try:
        ...
        with open(STEP1_IMAGE, "rb") as photo:
            msg = await update.message.reply_photo(
                photo=photo,
                caption=STEP1_TEXT.format(name=name),
    ...
    try:
        ...
        if data == "get_access":
            ...
            with open(STEP4_IMAGE, "rb") as photo:
                msg = await query.message.reply_photo(
                    photo=photo,
                    caption=STEP4_TEXT.format(name=name),
            ...
            if step == 2:
                ...
                with open(STEP3_IMAGE, "rb") as photo:
                    msg = await query.message.reply_photo(
                        photo=photo,
                        caption=STEP3_TEXT.format(name=name),
    ...
    try:
        ...
        if step == 2:
            ...
            with open(STEP3_IMAGE, "rb") as photo:
                msg = await update.message.reply_photo(
                    photo=photo,
                    caption=STEP3_TEXT.format(name=text),
                    reply_markup=step3_keyboard(),
                )
            context.user_data["step_messages"] = [msg.message_id]
            context.user_data["chat_id"] = update.effective_chat.id
...
async def on_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        with open(FINAL_IMAGE, "rb") as photo:
            msg = await update.message.reply_photo(
                photo=photo,
                caption=FINAL_TEXT.format(name=name),
